...
so today's SQLGlot engine (:class:`~dbt_column_lineage.artifacts.registry.ModelRegistry`)
...
class LineageProvider(Protocol):
    ...
    via :class:`~dbt_column_lineage.lineage.sqlglot_provider.SqlglotLineageProvider`); a
    ...
    def get_dialect(self) -> Optional[str]:
        ...
        Consumed by the AST semantic-diff (:mod:`~dbt_column_lineage.lineage.changeset`)
